...
/*   Updated: 2023/05/25 14:21:07 by gpimenta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	verify_color(char *str, char c, int i, int flag)
{
	...
	if (colors)
		rgb = ft_split(colors, ',');
	if (!verify_number(rgb) || !rgb_allocator(rgb, c))
		flag = 0;
	ft_free(rgb, colors);
	return (flag);
}
...
int	colors_allocator(char **all)
{
	...
	while (all[++i] && i < cub()->map_line)
	{
		...
		{
			if (!verify_color(all[i] + skip_spaces(all, i, 0), 'f', 2, 0))
				return (0);
		}
		else if (!ft_strncmp(all[i] + skip_spaces(all, i, 0), "C ", 2))
		{
			if (!verify_color(all[i] + skip_spaces(all, i, 0), 'c', 2, 0))
				return (0);
		}
	}
